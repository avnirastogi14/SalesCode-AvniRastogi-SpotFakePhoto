...
def removeCenter(magnitude: np.ndarray, radius: int=10,)-> np.ndarray:
    mag= magnitude.copy()
    h, w=mag.shape
    cy=h
    cx=w 
...
def midFrequencyRatio(magnitude: np.ndarray,) -> float:
    ...
    r = np.sqrt((x-cx)**2 + (y-cy))
    rMaxi = r.max()
    
    l = magnitude[r < rMaxi * 0.15].sum()

    mid = magnitude[(r >= rMaxi * 0.15) & (r<rMaxi*0.45)].sum()
    h = magnitude[r>= rMaxi*0.45].sum()
    return float(mid/(l+h+1e-6))
