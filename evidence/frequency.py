...
def radialSummary(magnitude: np.ndarray, bins: int = 25,) -> dict:
    # Collapse the 25-bin radial energy curve into a few summary stats
    # instead of dumping all bins as raw features (too high-dimensional
    # for a ~100 image dataset -> causes overfitting).
    energy = radialE(magnitude, bins)

    slope = float(np.polyfit(np.arange(bins), energy, 1)[0])

    return {
        "radial_energy_mean": float(np.mean(energy)),
        "radial_energy_std": float(np.std(energy)),
        "radial_energy_slope": slope,
    }
...
def freqE(gray: np.ndarray,):
    ...
    hrmc = harmonicCount(magnitude)
    reg = peakreg(magnitude)
    summary = radialSummary(magnitude)
    ...
    return {
        ...
        "features": {
            ...
            "harmonic_count": int(hrmc),
            "peak_reg": float(reg),
            "radial_energy_mean": summary["radial_energy_mean"],
            "radial_energy_std": summary["radial_energy_std"],
            "radial_energy_slope": summary["radial_energy_slope"],
        },
    }